...
def data_split(df, start, end, target_date_col="date"):
    ...
    if config.model_alive:
        data = data.sort_values([target_date_col, "tic"], ignore_index=True, ascending=False)
    else:
...
class FeatureEngineer:
    ...
    def add_vix(self, data):
        ...
        df_vix = YahooDownloader(
            start_date=df.date.min(), end_date=last_date, ticker_list=["^VIX"]
        ).fetch_data()
        vix = df_vix[["date", "close"]]
        vix.columns = ["date", "vix"]
        print("vix dataframe is ", vix)
    ...
    def calculate_turbulence(self, data):
        ...
        print("clean after using")
        print("turbulence index ", len(turbulence_index), turbulence_index, "**************   ", df_price_pivot.index)
